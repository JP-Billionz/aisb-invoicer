...
build is green, and a `render.yaml` Blueprint defines the Web Service
(Render free tier) — Postgres is externalized to Neon free tier
because Render removed their free Postgres plan in 2024. Deploy is a
10-minute click-through (create Neon DB, apply Blueprint, paste three
env vars). I'll update this file with the staging URL once you confirm.
...
login + a Neon DB provisioned + env vars pasted, none of which I have
credentials for. The Blueprint + Neon free tier keeps Phase 1A on a
no-card-on-file footprint:

1. Create a free Neon Postgres project at neon.tech (region: AWS US East
   Ohio). Copy the pooled connection string.
2. Render Dashboard → **New** → **Blueprint** → connect
   `JP-Billionz/aisb-invoicer` → Apply (creates the free Web Service).
3. Web Service → Environment → set `DATABASE_URL` (the Neon string),
   `NEXTAUTH_URL` (`https://aisb-invoicer.onrender.com`), leave
   `SENDGRID_API_KEY` blank.
4. Save → ~5 minute build (runs `prisma migrate deploy` against Neon).
5. `curl https://aisb-invoicer.onrender.com/api/health` should return
   `{"ok":true,...}`.
6. Paste the staging URL into our chat and I'll update this file.
...
1. **SendGrid wire-up** — flip the `if (SENDGRID_API_KEY)` branch in
   `src/lib/email.ts` (already written, just needs the key).
2. **Upgrade to paid tiers before first customer** — Web Service free →
   Starter ($7/mo, always-on); decide Neon free → Neon Launch ($19/mo)
   vs. Render Basic-256mb Postgres ($7/mo).
3. **Stripe billing**:
